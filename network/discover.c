...
//#define DC_DEBUG
#ifdef DC_DEBUG
#define eprintf(fmt, args...) printf("[%s](%d)=>" fmt, __FUNCTION__, __LINE__, ## args)
#else
...
void parseForHostUniq(UINT16_t type, UINT16_t len, unsigned char *data, void
    *extra)
{
    int *val = (int*)extra;
...
int listen_socket(unsigned int ip, int port, char *inf)
{
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, (char*) &interface, sizeof
        (interface)) < 0)
    {
        close(fd);
        return  - 1;
...
int raw_packet(struct dhcpMessage *payload, u_int32_t source_ip, int
    source_port, u_int32_t dest_ip, int dest_port, unsigned char *dest_arp, int
    ifindex)
{
    int fd;
    int result;
...
int get_raw_packet(struct dhcpMessage *payload, int fd)
{
    ...
    if (packet.ip.protocol != IPPROTO_UDP || packet.ip.version != IPVERSION ||
        packet.ip.ihl != sizeof(packet.ip) >> 2 || packet.udp.dest != htons
        (CLIENT_PORT) || bytes > (int)sizeof(struct udp_dhcp_packet) || ntohs
        (packet.udp.len) != (short)(bytes - sizeof(packet.ip)))
    {
    ...
    memcpy(payload, &(packet.data), bytes - (sizeof(packet.ip) + sizeof
        (packet.udp)));
...
int add_simple_option(unsigned char *optionptr, unsigned char code, u_int32_t
    data)
{
    char length = 0;
    int i;
...
int read_interface(char *interface, int *ifindex, u_int32_t *addr, unsigned
    char *arp)
{
    int fd;
    //struct ifreq ifr;
...
int discover_all( void )
{
    ...
    //sprintf( s_cmd, "iptables -I INPUT -i %s -p udp --sport 67 --dport 68 -j ACCEPT",
    ...
    if (read_interface(client_config.interface, &client_config.ifindex, NULL,
        client_config.arp) < 0)
    {
    ...
    for (;;)
    {
...
#ifdef DHCP_DETECT
	#if 1 //DEBUG
		eprintf("--- discover_all: FD_ISSET=%d---\n", FD_ISSET(1, &rfds) );
		eprintf("--- discover_all: FD_ISSET=%d---\n", FD_ISSET(2, &rfds) );
		eprintf("--- discover_all: FD_ISSET=%d---\n", FD_ISSET(3, &rfds) );
		eprintf("--- discover_all: FD_ISSET=%d---\n", FD_ISSET(4, &rfds) );
		eprintf("--- discover_all: FD_ISSET=%d---\n", FD_ISSET(5, &rfds) );
        ...
        if (retval > 0 && listen_mode != LISTEN_NONE && got_DHCP == 1)
        {
            ...
            if ((len < 0) || (packet.xid != xid)
                || ((message = get_option(&packet, DHCP_MESSAGE_TYPE)) == NULL))
            {
                ++count;
...
done_final:
    //sprintf( s_cmd, "iptables -D INPUT -i %s -p udp --sport 67 --dport 68 -j ACCEPT",
    //    nvram_safe_get( "wan_ifname" ) );
    //system( s_cmd );
